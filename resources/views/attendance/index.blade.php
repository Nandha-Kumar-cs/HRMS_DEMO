...
<div class="card page-card">
    ...
    <div class="card-body">
        ...
        {{-- All non-working days: modal triggered by JS below --}}
        @elseif($isNonWorking)
        {{-- popup handles both public holidays and weekends --}}
        @endif
                ...
                <table class="table table-bordered table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        @php
                            ...
                            $currentStatus = $attendanceMap[$emp->id] ?? null;
                            $hasTime       = !empty($rec?->check_in) || !empty($rec?->check_out);
                            $manualStatuses = ['absent' => 'Absent', 'on_leave' => 'On Leave', 'comp_off' => 'Comp Off', 'on_duty' => 'On Duty'];
                            $autoStatuses   = ['present' => 'Present', 'late' => 'Late', 'half_day' => 'Half Day'];
                            $autoBadgeColor = ['present' => 'success', 'late' => 'info', 'half_day' => 'warning text-dark'];
                            $safeStatus    = $currentStatus ?? 'absent';
                        @endphp
                        <tr data-emp-id="{{ $emp->id }}"
                            ...
                            <td style="min-width:130px">
                                {{-- Hidden input always submits the active status --}}
                                <input type="hidden" name="attendance[{{ $emp->id }}]"
                                       class="status-value" value="{{ $safeStatus }}">

                                {{-- Manual dropdown: visible only when no In/Out time --}}
                                <select class="form-select form-select-sm status-manual-select {{ $hasTime ? 'd-none' : '' }}">
                                    @foreach($manualStatuses as $val => $label)
                                        <option value="{{ $val }}"
                                            {{ (!$hasTime && $safeStatus === $val) ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>

                                {{-- Auto badge: visible when In/Out time exists --}}
                                <div class="status-auto-display {{ $hasTime ? '' : 'd-none' }}">
                                    <span class="badge status-auto-badge bg-{{ $autoBadgeColor[$safeStatus] ?? 'secondary' }}"
                                          style="font-size:.78rem;padding:.35em .65em;letter-spacing:.3px">
                                        {{ $autoStatuses[$safeStatus] ?? ucfirst($safeStatus) }}
                                    </span>
                            ...
                            <td>
                                <input type="time" name="check_in[{{ $emp->id }}]"
                                    class="form-control form-control-sm checkin-input"
                                    value="{{ $rec?->check_in ? \Illuminate\Support\Str::substr($rec->check_in, 0, 5) : '' }}">
                            </td>
                            <td>
                                <input type="time" name="check_out[{{ $emp->id }}]"
                                    class="form-control form-control-sm checkout-input"
                                    value="{{ $rec?->check_out ? \Illuminate\Support\Str::substr($rec->check_out, 0, 5) : '' }}">
                            </td>
                            <td>
                                @if($emp->ot_enabled)
...
@php
    if ($holidayRecord) {
        $popupName = $holidayRecord->name;
        $popupType = $holidayRecord->holidayType?->name ? $holidayRecord->holidayType->name . ' Holiday' : 'Public Holiday';
        $popupIcon = 'fa-calendar-xmark';
        $popupColor = '#f59e0b';
    } else {
        $popupName = explode(' — ', $nonWorkingLabel ?? '')[0];
        $popupType = 'Weekly Off';
        $popupIcon = 'fa-moon';
        $popupColor = '#64748b';
    }
@endphp
    ...
    <div class="modal-dialog modal-dialog-centered" style="max-width:520px">
        <div class="modal-content border-0 shadow-lg" style="border-radius:14px;overflow:hidden">
            <div class="modal-body text-center px-5 py-4">
                ...
                <div class="badge mb-2 px-3 py-1" style="background:{{ $popupColor }}22;color:{{ $popupColor }};font-size:.75rem;font-weight:600;border-radius:20px">
                    {{ $popupType }}
                </div>
                ...
                <a href="{{ route('attendance.index') }}?date={{ $prevDate }}"
...
$(function(){
    ...
    var OFFICE_START_MINS = {{ \App\Helpers\AppSettings::getOfficeStartMins() }};
    var DAILY_GRACE_MINS  = {{ \App\Helpers\AppSettings::getDailyGraceMinutes() }};
    var LATE_THRESHOLD    = OFFICE_START_MINS + DAILY_GRACE_MINS;
    var OT_TRIGGER_MINS   = {{ \App\Helpers\AppSettings::getOtTriggerMins() }};
    var OT_BASELINE_MINS  = {{ \App\Helpers\AppSettings::getOtBaselineMins() }};

    var MANUAL_STATUSES = { absent: 'Absent', on_leave: 'On Leave', comp_off: 'Comp Off', on_duty: 'On Duty' };
    var AUTO_LABELS     = { present: 'Present', late: 'Late', half_day: 'Half Day' };
    var BADGE_CLASS     = { present: 'bg-success', late: 'bg-info', half_day: 'bg-warning text-dark' };

    // ── Status auto-calculation ────────────────────────────────────────────────
    var HALF_DAY_CHECKIN = 11 * 60; // check-in at or after 11:00 AM → half day
    ...
    function calcAutoStatus(checkIn, checkOut) {
        ...
        // Check-in at or after 11:00 AM → always half day
        if (inMins >= HALF_DAY_CHECKIN) return 'half_day';
        // Has checkout but worked < 4 hours → half day
        if (checkOut) {
            var outParts = checkOut.split(':');
            var outMins  = parseInt(outParts[0], 10) * 60 + parseInt(outParts[1], 10);
            var workMins = outMins - inMins;
            if (workMins > 0 && workMins < 240) return 'half_day';
        }
        return inMins > LATE_THRESHOLD ? 'late' : 'present';
    }
    ...
    function updateRow($tr) {
        ...
        if (checkIn || checkOut) {
            ...
            $badge.attr('class', 'badge status-auto-badge ' + (BADGE_CLASS[status] || 'bg-secondary'))
                  .text(AUTO_LABELS[status] || status);
            $hidden.val(status);
            ...
            // ── Manual mode ────────────────────────────────────────────────────
            var cur = $hidden.val();
            if (!MANUAL_STATUSES[cur]) {
                cur = 'absent';
                $hidden.val(cur);
            }
            // Rebuild options if select currently has auto options
            if (!$manual.find('option[value="absent"]').length) {
                $manual.empty();
                $.each(MANUAL_STATUSES, function(val, label) {
                    $manual.append($('<option>').val(val).text(label));
                });
            }
            $manual.val(cur);
    ...
    $(document).on('change', '.status-manual-select', function() {
        $(this).closest('tr').find('.status-value').val($(this).val());
    });
    ...
    // ── Mark All buttons (only affect manual-mode rows) ───────────────────────
    $('#markAllAbsent').on('click', function() {
        $('tbody tr').each(function() {
            var $tr = $(this);
            if (!$tr.find('.status-manual-select').hasClass('d-none')) {
                $tr.find('.status-manual-select').val('absent').trigger('change');
            }
        });
    });
    $('#markAllLeave').on('click', function() {
        $('tbody tr').each(function() {
            var $tr = $(this);
            if (!$tr.find('.status-manual-select').hasClass('d-none')) {
                $tr.find('.status-manual-select').val('on_leave').trigger('change');
            }
        });
    });
    ...
    function calcAutoOT($tr, val) {
        ...
        if (!val) { $hoursInput.val(''); $hoursDisp.text('—'); return; }
        var parts   = val.split(':');
    ...
    $('#attendFile').on('change', function(){
        var f = this.files[0];
        if (f) { $('#attendFileName').text(f.name + ' (' + (f.size/1024).toFixed(1) + ' KB)'); $('#attendPreview').removeClass('d-none'); }
    });
    $('#attendImportForm').on('submit', function(){
        $('#btnAttendImport').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Importing…');
    });
    $('#monthlyFile').on('change', function(){
        var f = this.files[0];
        if (f) { $('#monthlyFileName').text(f.name + ' (' + (f.size/1024).toFixed(1) + ' KB)'); $('#monthlyPreview').removeClass('d-none'); }
    });
    $('#monthlyImportForm').on('submit', function(){
        $('#btnMonthlyImport').prop('disabled', true).html('<span class="spinner-border spinner-border-sm me-1"></span>Importing… this may take a moment');
    });

    // ── Initialise all rows on page load ──────────────────────────────────────
    $('tbody tr').each(function() { updateRow($(this)); });

    // ── Holiday / non-working day modal: auto-open on page load ─────────────
    @if($isNonWorking)
