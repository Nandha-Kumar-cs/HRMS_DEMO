...
<div class="page">
    ...
    {{-- Header --}}
    @php $slipEntity = $salarySlip->employee->entity; @endphp
    <div class="header">
        <div style="display:flex;align-items:center;gap:12px;">
            @if($slipEntity && $slipEntity->logo_base64)
                <img src="{{ $slipEntity->logo_base64 }}"
                     style="max-height:60px;max-width:80px;object-fit:contain;" alt="Logo">
            @endif
            <div>
                <div class="company-name">{{ $slipEntity?->name ?? 'HRMS' }}</div>
                @if($slipEntity)
                    @php
                        $slipAddr = collect([
                            $slipEntity->address ?? null,
                            $slipEntity->city    ?? null,
                            $slipEntity->state   ?? null,
                            $slipEntity->pincode ?? null,
                        ])->filter()->implode(', ');
                    @endphp
                    @if($slipAddr)<div class="company-sub">{{ $slipAddr }}</div>@endif
                @else
    ...
    <div class="footer">
        ...
        &bull; Generated by {{ $slipEntity?->name ?? 'HRMS' }} &bull; {{ now()->format('d M Y H:i') }}
    </div>
</div>
</body>
</html>
