...
<div class="card page-card">
    ...
    <div class="card-body">
        ...
        {{-- Date picker --}}
        <form method="GET" id="datePickerForm" class="row g-2 mb-3 align-items-end">
            <div class="col-auto">
                <label class="form-label fw-semibold mb-1">Date</label>
                <input type="date" name="date" id="dateInput" class="form-control" value="{{ $date }}" max="{{ today()->toDateString() }}">
            </div>
            <div class="col-auto">
        ...
        {{-- Holiday warning popup --}}
        <div class="modal fade" id="holidayWarnModal" tabindex="-1" aria-labelledby="holidayWarnLabel" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-sm">
                <div class="modal-content">
                    <div class="modal-header border-0 pb-0">
                        <h6 class="modal-title fw-semibold text-danger" id="holidayWarnLabel">
                            <i class="fa fa-calendar-xmark me-2"></i>Public Holiday
                        </h6>
                    </div>
                    <div class="modal-body pt-1">
                        <p class="mb-0" id="holidayWarnMsg"></p>
                    </div>
                    <div class="modal-footer border-0 pt-1">
                        <button type="button" class="btn btn-secondary btn-sm" data-bs-dismiss="modal">Stay on Current Date</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        {{-- Regular non-working day warning --}}
        @elseif($isNonWorking && $nonWorkingLabel)
        <div class="alert alert-warning d-flex align-items-center gap-2 mb-3 py-2">
            ...
            <div>
                ...
                <a href="{{ route('comp-off.working-days') }}"
                   class="btn btn-sm btn-outline-warning py-0 px-2 ms-2" style="font-size:.78rem">
                    <i class="fa fa-plus me-1"></i>Declare as Comp Off Working Day
                </a>
            </div>
        </div>
        @endif
...
@push('scripts')
<script>
// Holiday dates pre-loaded from DB: { "2025-01-14": "Makar Sankranti", ... }
var HOLIDAY_MAP = @json($holidayMap);

$(function(){
    var CURRENT_DATE = '{{ $date }}';

    // Intercept date-picker form: if selected date is a holiday, show popup and cancel navigation
    $('#datePickerForm').on('submit', function(e) {
        var selected = $('#dateInput').val();
        if (selected && HOLIDAY_MAP[selected]) {
            e.preventDefault();
            $('#holidayWarnMsg').text(
                '”' + HOLIDAY_MAP[selected] + '” is a public holiday. ' +
                'Attendance entry is not required.'
            );
            $('#holidayWarnModal').modal('show');
        }
    });

    // Reset date picker to the currently loaded date when holiday modal is dismissed
    $('#holidayWarnModal').on('hidden.bs.modal', function() {
        $('#dateInput').val(CURRENT_DATE);
    });
