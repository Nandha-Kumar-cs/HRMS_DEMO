...
class AttendanceController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Holiday map for client-side date-picker intercept: date => name
        $holidayMap = \App\Models\Holiday::pluck('name', 'date')
            ->mapWithKeys(fn($name, $d) => [
                \Carbon\Carbon::parse($d)->toDateString() => $name
            ])
            ->toArray();

        // Also include comp-off working days (declared working holidays)
        $compOffWorkingDay = \App\Models\CompOffWorkingDay::where('date', $date)->first();

        return view('attendance.index', compact(
            'date', 'employees', 'attendanceMap', 'isNonWorking', 'nonWorkingLabel',
            'holidayMap', 'compOffWorkingDay'
        ));
    }
