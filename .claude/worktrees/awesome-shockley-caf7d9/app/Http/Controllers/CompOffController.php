...
class CompOffController extends Controller
{
    ...
    /** Remove ALL comp offs for a holiday date and revert auto-created attendance records */
    public function bulkRemove(Request $request)
    {
        ...
        $compOffs = CompOff::where('holiday_date', $request->holiday_date)->get();

        foreach ($compOffs as $co) {
            if ($co->status === 'availed' && $co->availed_date) {
                // Remove the attendance record that was auto-created by bulkAvail
                \App\Models\Attendance::where('employee_id', $co->employee_id)
                    ->where('date', $co->availed_date)
                    ->where('status', 'comp_off')
                    ->where('remarks', 'like', 'Comp Off — %')
                    ->delete();
            }
        }

        $count = $compOffs->count();
        CompOff::where('holiday_date', $request->holiday_date)->delete();

        return back()->with('success', "{$count} comp off record(s) removed. Related attendance entries have been reverted.");
    }
