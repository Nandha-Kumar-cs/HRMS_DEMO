            ...
            <div class="card-body">
                <table class="table table-borderless table-sm">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ Storage::disk('public')->url($leaveRequest->document) }}"
                               target="_blank"
                               class="btn btn-sm btn-outline-primary">
                                <i class="fa fa-paperclip me-1"></i>View Attachment
                            </a>
                            <small class="text-muted ms-2">{{ basename($leaveRequest->document) }}</small>
                        </td>
                    </tr>
                    @endif
...
@push('scripts')
<script>
document.getElementById('deleteLeaveBtn')?.addEventListener('click', function () {
    Swal.fire({
        title: 'Delete Leave Request?',
        html: 'This will permanently delete the request.<br>@if($leaveRequest->status === "approved")<strong>Approved leave:</strong> balance and attendance records will be reversed.@endif',
        icon: 'warning',
        ...
        if (result.isConfirmed) document.getElementById('deleteLeaveForm').submit();
    });
});
</script>
@endpush
