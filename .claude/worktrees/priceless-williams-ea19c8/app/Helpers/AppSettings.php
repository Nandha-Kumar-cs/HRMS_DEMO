...
class AppSettings
{
    ...
    /**
     * Daily late grace in minutes (default 15).
     * Check-in within this window after office start is NOT counted as late.
     * But if late, minutes are measured from office start, not from grace cutoff.
     */
    public static function getDailyGraceMinutes(): int
    {
        return (int) self::get('daily_grace_minutes', 15);
    }

    /**
     * Monthly late permission in minutes (default 90 = 1h 30m).
     * Total late minutes ≤ this → no deduction.
     * Total late minutes > this → 2× full-amount deduction.
     */
    public static function getMonthlyGraceMinutes(): int
    {
        return (int) self::get('monthly_grace_minutes', 90);
    }
