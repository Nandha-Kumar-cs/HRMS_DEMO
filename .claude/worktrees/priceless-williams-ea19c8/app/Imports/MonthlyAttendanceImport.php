...
class MonthlyAttendanceImport implements ToCollection
{
    ...
    private function resolveStatus(?string $checkIn, ?float $workHours): string
    {
        ...
        // Half day: worked > 0 but < 4 hours (checkout exists but short shift)
        if ($hours > 0.0 && $hours < 4.0) return 'half_day';

        // Late: checked in after (office_start + daily_grace)
        // Determined purely by check-in time — work hours are irrelevant for late detection.
        // e.g. office 09:00 + grace 15 min → late if check-in > 09:15
        $officeStartMins = AppSettings::getOfficeStartMins();
        $dailyGraceMins  = AppSettings::getDailyGraceMinutes();
        $lateThreshold   = $officeStartMins + $dailyGraceMins;
