...
<div class="card page-card">
    <div class="card-header bg-white py-3 d-flex align-items-center gap-2">
        ...
        <h5 class="mb-0 fw-semibold">Edit Loan / Advance</h5>
    </div>
    <div class="card-body">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <select name="employee_id" class="form-select">
                        @foreach($employees as $emp)<option value="{{ $emp->id }}" {{ old('employee_id', $loan->employee_id) == $emp->id ? 'selected' : '' }}>{{ $emp->full_name }} ({{ $emp->employee_code }})</option>@endforeach
                    </select>
                </div>
                <div class="col-md-3">
                    ...
                    <select name="type" class="form-select">
                        ...
                        <option value="advance" {{ old('type', $loan->type) === 'advance' ? 'selected' : '' }}>Advance</option>
                    </select>
                </div>
                <div class="col-md-3">
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="amount" id="inpAmount" step="0.01" min="1" class="form-control" value="{{ old('amount', $loan->amount) }}" required></div>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Interest Rate (% / year)</label>
                    <div class="input-group">
                    <input type="number" name="interest_rate" id="inpRate" step="0.01" min="0" class="form-control" value="{{ old('interest_rate', $loan->interest_rate) }}">
                    <span class="input-group-text">%</span></div>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Date Given</label>
                    <input type="date" name="date_given" class="form-control" value="{{ old('date_given', $loan->date_given->format('Y-m-d')) }}" required>
                </div>
                <div class="col-md-3">
                    <label class="form-label">Total Months</label>
                    <input type="number" name="total_months" id="inpMonths" min="1" class="form-control" value="{{ old('total_months', $loan->total_months) }}" required>
                </div>

                {{-- Interest summary --}}
                <div class="col-12">
                    <div id="loanSummary" class="alert alert-info py-2 small">
                        <div class="row g-1">
                ...
                <div class="col-md-3">
                    <label class="form-label">Monthly Deduction (EMI)</label>
                    <div class="input-group"><span class="input-group-text">₹</span>
                    <input type="number" name="monthly_deduction" id="inpEmi" step="0.01" min="0.01" class="form-control" value="{{ old('monthly_deduction', $loan->monthly_deduction) }}" required></div>
                ...
                <div class="col-12">
                    ...
                    <textarea name="remarks" class="form-control" rows="2">{{ old('remarks', $loan->remarks) }}</textarea>
                </div>
            </div>
            <div class="mt-4 d-flex gap-2">
                <button type="submit" class="btn btn-primary"><i class="fa fa-save me-1"></i>Update</button>
                <a href="{{ route('loans.show', $loan) }}" class="btn btn-light">Cancel</a>
            </div>
        </form>
    </div>
</div>
