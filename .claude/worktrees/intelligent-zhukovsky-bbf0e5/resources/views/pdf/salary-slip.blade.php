...
<style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .header { display: flex; justify-content: space-between; align-items: flex-start;
               border-bottom: 3px solid #2563eb; padding-bottom: 12px; margin-bottom: 14px; }
    ...
    .emp-cell { width: 33.33%; margin-bottom: 7px; }
    ...
    .dual-table { width: 100%; border-collapse: collapse; margin-bottom: 12px; }
    .dual-table td, .dual-table th { padding: 5px 8px; border: 1px solid #e2e8f0; font-size: 8.5pt; vertical-align: top; }
    ...
    /* Net box */
    .net-box { background: #1e3a5f; color: #fff; padding: 10px 14px; border-radius: 5px;
               display: flex; justify-content: space-between; align-items: center; margin-bottom: 14px; }
    .net-label  { font-size: 11pt; font-weight: bold; }
    .net-amount { font-size: 14pt; font-weight: bold; }

    /* Attendance summary */
    .att-box { border: 1px solid #e2e8f0; border-radius: 5px; padding: 10px 14px; margin-bottom: 12px; }
    .att-title { font-size: 8.5pt; font-weight: bold; color: #475569; margin-bottom: 8px;
                 padding-bottom: 5px; border-bottom: 1px solid #e2e8f0; }
    .att-grid  { display: flex; flex-wrap: wrap; }
    .att-cell  { width: 16.66%; text-align: center; padding: 4px 0; }
    .att-val   { font-size: 12pt; font-weight: bold; }
    .att-lbl   { font-size: 7pt; color: #64748b; }
    ...
    /* OT / Late row */
    .extra-row { display: flex; flex-wrap: wrap; border-top: 1px solid #e2e8f0; padding-top: 8px; margin-top: 6px; }
    .extra-cell { width: 25%; text-align: center; padding: 4px 0; }
    .extra-val  { font-size: 10pt; font-weight: bold; }
    .extra-lbl  { font-size: 7pt; color: #64748b; }

    /* Rates row */
    .rates { font-size: 7.5pt; color: #64748b; margin-top: 6px; display: flex; gap: 24px; }
...
<div class="page">
    ...
    <div class="att-box">
        <div class="att-title">Attendance Summary — {{ $salarySlip->month_name }} {{ $salarySlip->year }}</div>
        <div class="att-grid">
            <div class="att-cell"><div class="att-val c-blue">{{ $att['total_working_days'] }}</div><div class="att-lbl">Working Days</div></div>
            <div class="att-cell"><div class="att-val c-green">{{ $att['present_days'] }}</div><div class="att-lbl">Present</div></div>
            <div class="att-cell"><div class="att-val c-yellow">{{ $att['half_days'] }}</div><div class="att-lbl">Half Day</div></div>
            <div class="att-cell"><div class="att-val c-gray">{{ $att['leave_days'] }}</div><div class="att-lbl">On Leave</div></div>
            <div class="att-cell"><div class="att-val c-red">{{ $att['absent_days'] }}</div><div class="att-lbl">Absent</div></div>
            <div class="att-cell"><div class="att-val c-yellow">{{ $att['late_days'] ?? 0 }}</div><div class="att-lbl">Late Days</div></div>
        </div>
        ...
        <div class="extra-row">
            ...
            <div class="extra-cell">
                <div class="extra-val c-green">{{ $att['ot_hours'] }} hrs</div>
                <div class="extra-lbl">OT Hours (1.5×)</div>
            </div>
            <div class="extra-cell">
                <div class="extra-val c-green">&#8377; {{ number_format($att['ot_amount'] ?? 0, 2) }}</div>
                <div class="extra-lbl">OT Pay</div>
            </div>
            @endif
            @if(($att['late_minutes'] ?? 0) > 0)
            <div class="extra-cell">
                <div class="extra-val c-yellow">{{ $att['late_minutes'] }} min</div>
                <div class="extra-lbl">Total Late Minutes</div>
            </div>
            <div class="extra-cell">
                <div class="extra-val c-red">&#8377; {{ number_format($att['late_deduction'] ?? 0, 2) }}</div>
                <div class="extra-lbl">Late Deduction</div>
            </div>
            @endif
        </div>
        @endif
        ...
        <div class="rates">
            ...
            <span>Basic: <strong>&#8377; {{ number_format($att['basic_salary'], 2) }}</strong></span>
            @endif
            <span>LOP/Day: <strong>&#8377; {{ number_format($att['per_day_salary'] ?? 0, 2) }}</strong></span>
            @if(isset($att['per_hour_rate']))
            <span>OT Rate (2×): <strong>&#8377; {{ number_format(($att['per_hour_rate'] ?? 0) * 2, 2) }}</strong></span>
            @endif
        </div>
    </div>
    @endif
