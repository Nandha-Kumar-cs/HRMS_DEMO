...
class SalarySlipController extends Controller
{
    ...
    {
        $slips = SalarySlip::with('employee')->latest()->paginate(15);
        return view('salary-slips.index', compact('slips'));
    }
    ...
    public function store(StoreSalarySlipRequest $request)
    {
        ...
        // BUG-4 FIX: Only process loans that were given on or before the payroll
        // month end.  Future loans must NOT appear in historical payslips.
    ...
    public function destroy(SalarySlip $salarySlip)
    {
        ...
        foreach ($salarySlip->loanRepayments as $repayment) {
            ...
            if ($loan && $loan->paid_months > 0) {
                ...
                // Re-open closed loans that had their final payment reversed
    ...
    private function computePayroll(
        ...
        $presentDays = $attendanceRecords->whereIn('status', ['present', 'late'])->count() - $noCheckoutAbsent;
        ...
        // Step B — find all approved leave-request working-days for this month
        $approvedLeaveDates = []; // date string => true
        $approvedLeaveDays  = 0;
        ...
        if ($month && $year) {
            ...
            $paidLeaveRequests = LeaveRequest::with('leaveType')
            ...
            foreach ($paidLeaveRequests as $req) {
                $cur = Carbon::parse(max((string) $req->start_date, $mStart));
                $end = Carbon::parse(min((string) $req->end_date,   $mEnd));
                while ($cur->lte($end)) {
                    $ds = $cur->toDateString();
                    if (!isset($approvedLeaveDates[$ds]) && WorkCalendar::isWorkingDay($cur)) {
                        $approvedLeaveDates[$ds] = true;
                        $approvedLeaveDays++;
                    }
                    $cur->addDay();
                }
        ...
        // Step C — count on_leave attendance records NOT covered by paid requests
        $unapprovedLeaveDays = 0;
        foreach ($leaveDateStrings as $ds) {
            if (!isset($approvedLeaveDates[$ds])) {
                $unapprovedLeaveDays++;
            }
        }

        $freeMonthlyQuota = 1; // 1 free paid leave per month per employee
        $freeLeaveUsed    = min($freeMonthlyQuota, $unapprovedLeaveDays);
        $unpaidLeaveDays  = max(0, $unapprovedLeaveDays - $freeLeaveUsed);
        $paidLeaveDays    = $approvedLeaveDays + $freeLeaveUsed;

        // Unpaid leaves are treated like absent days — deducted from salary.
        // The formula: absentDays = totalWorkingDays - presentDays - halfDays/2 - paidLeaveDays
        // This correctly counts every unaccounted working day as absent.
        ...
        $lateMinutes  = 0;
        $cutoffMinsSS = 9 * 60;  // 540 = 09:00 office start (minutes-since-midnight)
        ...
        // ── Step 5b: Employee Benefit Funds (active for this month) ──────────
        $benefits      = [];
        $benefitsTotal = 0;
        if ($month && $year) {
            ...
            foreach ($activeBenefits as $b) {
                $key = $b->fundType?->name ?? 'Benefit Fund';
                if (isset($benefits[$key])) {
                    $key .= ' #' . $b->id;
                }
                $benefits[$key] = (float) $b->amount;
                $benefitsTotal  += (float) $b->amount;
            }
        }
        ...
        $perDayRate    = $calendarDays > 0 ? round($ctcPerMonth / $calendarDays, 4) : 0;
        ...
        // ── Late deduction: 90-minute monthly grace, 2× penalty ─────────────
        // • ≤ 90 min total late in month  → no deduction at all
        // • > 90 min total late in month  → deduct TOTAL late × 2
        ...
        if ($absentDeduction > 0) {
            ...
            if ($unpaidLeaveDays > 0) {
                $label .= ', incl. ' . $unpaidLeaveDays . ' unpaid leave';
            }
            $label .= ')';
        ...
        // BUG-4 FIX: Only include loans that were disbursed on or before the last
        // day of this payroll month.  Loans taken in future months must NOT bleed
        // into historical payslips.
