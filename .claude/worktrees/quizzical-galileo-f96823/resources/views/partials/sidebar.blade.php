<nav id="sidebar">
    ...
    <ul class="sidebar-nav">
        ...
        {{-- ── ATTENDANCE ──────────────────────────────── --}}
        @php $attActive = request()->routeIs('attendance.*','leave-requests.*','leave-status.*','leave-types.*','holidays.*','comp-offs.*','on-duties.*'); @endphp
        <li>
        ...
        @php $settActive = request()->routeIs('salary-components.*','departments.*','designations.*','entities.*','leave-types.*','users.*','holiday-types.*','benefit-fund-types.*','roles.*','permissions.*','mobile-access.*','settings.ot.*'); @endphp
        <li>
