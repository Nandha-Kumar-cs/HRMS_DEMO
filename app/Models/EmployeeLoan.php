...
class EmployeeLoan extends Model
{
    ...
    /**
     * The last calendar month in which a deduction should be made.
     * Deductions run for total_months installments starting from the month
     * AFTER disbursement (e.g. given May → Jun, Jul, Aug for 3 months).
     * Returns a Carbon set to the 1st of the final deduction month.
     */
    public function getFinalDeductionMonthAttribute(): Carbon
    {
        return Carbon::parse($this->date_given)->startOfMonth()->addMonths($this->total_months);
    }

    /**
     * Whether this loan should still be deducted in a given payroll month/year.
     * Returns false once the payroll month is past the final deduction month.
     */
    public function isActiveForPayroll(int $payrollMonth, int $payrollYear): bool
    {
        if (!in_array($this->status, ['active', 'closed'])) return false;
        if ($this->date_given && Carbon::create($payrollYear, $payrollMonth, 1)
                ->gt($this->final_deduction_month)) {
            return false;
        }
        return true;
    }
